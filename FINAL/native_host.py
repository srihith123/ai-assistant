#!/Users/ansh_is_g/.pyenv/versions/3.10.1/bin/python3
"""
...
# Define tutor prompt for direct interaction
TUTOR_PROMPT = """
You are an intelligent, engaging, and compassionate AI tutor with expertise in mathematics and English. Your primary role is to help students understand concepts and solve problems. You must:

1. Analyze the image in front of you.
2. Begin by describing what you see in clear, human-like language.
3. Use only natural, conversational language—avoid technical notations.
4. Provide clear, relatable examples using everyday language.
3. Use only natural, conversational language—completely avoid any technical notations, code-style formatting, or LaTeX. For instance, articulate “x minus four” instead of “x-4”, and say “five times three” instead of “5 * 3.” when appropriate.
4. Engage in a step-by-step dialogue, asking thoughtful questions that encourage the student to think through each part of the problem.
5. Provide clear, relatable examples when explaining difficult concepts, using everyday language that makes sense out loud.
6. Adjust your explanations to the student’s pace and needs, checking in frequently to ensure understanding, and avoid giving complete answers unless the student specifically asks for them.
7. Empower the student to discover the answer by verifying each step through inquiry and discussion—never rush to a final solution without building the student’s conceptual understanding.

Start by carefully analyzing the image in front of you. Offer an initial audio explanation that describes the problem, suggests possible approaches, and invites the student to engage by asking, “What do you think might be the best way to start?”.
"""
        ...
        try:
            ...
            async with self.client.aio.live.connect(model=MODEL_NAME, config=LIVE_CONFIG) as session:
                ...
                try:
                    ...
                    logger.info("Session tasks cancelled, cleaning up...")
                    for task in tasks:
                        if not task.done():
                            task.cancel()
    ...
    async def main_loop(self):
        ...
        try:
            while True:
                ...
                elif message_type == "interrupt_playback":
                    logger.info("Interrupt playback requested")
                    self.interrupt_playback_event.set()
                    await asyncio.sleep(0.1)  # Brief pause to let interruption process
                ...
                # Add handling for reset_state message
                elif message_type == "reset_state":
                    logger.info("Received reset_state request, preparing for next question...")
                    if gemini_session_task and not gemini_session_task.done():
                        gemini_session_task.cancel()
                        try:
                            await gemini_session_task
                    ...
                    # Reset state but maintain connection
                    self.session = None
                    self.initial_image_sent = False
                    self.is_mic_muted = True
                    ...
                    send_native_message({"type": "reset_complete", "success": True})
                    logger.info("Reset complete, ready for next question")
            ...
            if self.pya:
                ...
                await asyncio.sleep(0.2)  # Small delay to allow audio tasks to clean up
