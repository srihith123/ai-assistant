...
const TEST_HOST_NAME = "com.bomboclatt.test_host"; // Test host name
let nativePort = null;
...
function testNativeMessaging() {
    console.log(`Attempting to connect to test host: ${TEST_HOST_NAME}`);
    try {
        const testPort = chrome.runtime.connectNative(TEST_HOST_NAME);
        testPort.onMessage.addListener((message) => {
            console.log("Test host response:", message);
            testPort.disconnect();
        });
        ...
        // Send a test message
        testPort.postMessage({test: "hello"});
    } catch (e) {
        console.error(`Failed to connect to test host: ${e}`);
    }
}
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    } else if (request.type === "mute_mic" || request.type === "unmute_mic") {
        console.log(`Background: Forwarding ${request.type} to native host.`);
        sendToNativeHost({ type: request.type }); // Forward the message directly
